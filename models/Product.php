...
class Product extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['shop_id', 'article', 'name', 'description', 'url', 'cover_tmp'], 'required'],
            [['shop_id', 'collection_id', 'category_id', 'manual_id', 'coat_id', 'drawing_id', 'cover_id', 'length', 'width', 'height', 'is_promotion'], 'integer'],
            [['article', 'name', 'description', 'url', 'meta_title', 'meta_description', 'meta_keywords'], 'string', 'max' => 255],
            [['line_ids', 'cover_tmp', 'manual_tmp', 'drawing_tmp'], 'safe']
        ];
    }
    ...
    public function beforeValidate()
    {
        if ($this->cover_tmp) {
            if (empty($this->cover_id)) {
                ...
                $upload = Upload::findOne($this->cover_id);
            }

            $upload->name = $this->cover_tmp;
            $upload->path = Upload::getUploadsPathByType(1, $upload->name);

            $source = Upload::getTmpUploadsPath() . $upload->name;
            $dest = Upload::getTmpUploadsPath() . $upload->path;
            if (!copy($source, $dest)) {
