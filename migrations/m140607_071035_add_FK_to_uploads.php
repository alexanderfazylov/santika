...
class m140607_071035_add_FK_to_uploads extends \yii\db\Migration
{
    public function safeUp()
    {
        ...
        $this->execute('Update {{%product}} set manual_id = NULL where photo_id NOT IN (Select id from {{%upload}})');
        $this->addForeignKey('FK_product_manual_to_upload', '{{%product}}', 'manual_id', '{{%upload}}', 'id');

        $this->execute('Update {{%product}} set drawing_id = NULL where photo_id NOT IN (Select id from {{%upload}})');
